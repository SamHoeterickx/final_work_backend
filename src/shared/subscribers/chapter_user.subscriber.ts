...
import { ChapterUser } from '../../modules/chapters/entity/chapter_user.entity';
  ...
  export class ChapterUserSubscriber implements EntitySubscriberInterface<ChapterUser> {
        ...
        async afterUpdate(event: UpdateEvent<ChapterUser>) {
            ...
            if (oldStatus === EProgressStatus.LOCKED && newStatus === EProgressStatus.UNLOCKED) {
                const chapter = event.entity?.chapter || event.databaseEntity?.chapter;
                const user = event.entity?.user || event.databaseEntity?.user;
                ...
                console.log(`Lessen unlocked voor user ${user.uuid} in chapter ${chapterUuid}`);
            }
        }
  }